...
def app():
    def _fileDownload(df, dname):
        '''Provide dataframe for download'''

        csv = df.to_csv(index=False)
        b64 = base64.b64encode(csv.encode()).decode()  # strings <-> bytes conversions
        href = f'<a href="data:file/csv;base64,{b64}" download="descriptor_{dname}.csv">Download CSV File</a>'
        return href
    ...
    def _runPaDEL():
        ...
        st.write(desc)
        st.markdown(_fileDownload(desc, user_fp), unsafe_allow_html=True)
    ...
    with st.sidebar.header('1. Upload your CSV data'):
        ...
        st.sidebar.markdown(_fileDownload(exemple_file, 'example'), unsafe_allow_html=True)
